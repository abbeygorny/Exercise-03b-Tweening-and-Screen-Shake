...
func generate(pos):
	...
	target_position = pos
	$Tween.interpolate_property(self, "position", position, target_position, randf()+8.5, Tween.TRANS_BOUNCE, Tween.EASE_OUT)
	$Tween.start()

func move_piece(change):
	target_position = target_position + change
	$Tween.interpolate_property(self, "position", position, target_position, randf()+8.5, Tween.TRANS_LINEAR, Tween.EASE_IN)
	$Tween.start()
...
func die():
	...
	if Effects != null:
		...
		$Timer.wait_time = 8.5 + (randf() / 10.0)
		$Timer.start()
